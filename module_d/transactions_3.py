...
class Transactions3(object):
    ...
    def __init__(self, imports, version, articles, tarifs):
        ...
        for entree in imports.acces.donnees:
            ...
            util_proj = self.util_proj(entree['id_user'], compte, pt['flow-cae'])
            counted = False
            ...
            if duree_hp > 0 or duree_hc > 0:
                ...
                if id_groupe != '0' and duree_op == 0:
                    ...
                    if self.plateforme['id_plateforme'] == article['platf-code']:
                        ref_client = self.ref_client(entree['annee'], entree['mois'], classe, client, compte, article)
                        tarif = tarifs.valeurs[id_classe + id_groupe]
                        art = self.art_plate(article, "K3", pt['item-K3'], pt['item-K3a'])
                ...
                if id_groupe != '0':
                    ...
                    if self.plateforme['id_plateforme'] == article['platf-code']:
                        ref_client = self.ref_client(entree['annee'], entree['mois'], classe, client, compte, article)
                        tarif = tarifs.valeurs[id_classe + id_groupe]
                        art = self.art_plate(article, "K7", pt['item-K7'], pt['item-K7a'])
                ...
                if id_groupe != '0':
                    ...
                    if prix_extra > 0:
                        article = articles.valeurs[id_groupe]
                        if self.plateforme['id_plateforme'] == article['platf-code']:
                            ref_client = self.ref_client(entree['annee'], entree['mois'], classe, client, compte,
                                                         article)
                            tarif = tarifs.valeurs[id_classe + id_groupe]
                            ...
                            art = self.art_plate(article, "K4", pt['item-K4'], pt['item-K4a'])
                    ...
                    if duree_hp > 0:
                        if article['platf-code'] == compte['code_client'] and compte['exploitation'] == "TRUE":
                            ...
                            if counted:
                                ...
                                runcae = 1
                                counted = True
                        art = self.art_plate(article, "K1", pt['item-K1'], pt['item-K1a'])
                    ...
                    if duree_hc > 0:
                        if article['platf-code'] == compte['code_client'] and compte['exploitation'] == "TRUE":
                            ...
                            if counted:
                                ...
                                runcae = 1
                                counted = True
                        art = self.art_plate(article, "K1", pt['item-K1'], pt['item-K1b'])
            ...
            if id_groupe != '0' and duree_op > 0:
                ...
                if self.plateforme['id_plateforme'] == article['platf-code']:
                    ref_client = self.ref_client(entree['annee'], entree['mois'], classe, client, compte, article)
                    tarif = tarifs.valeurs[id_classe + id_groupe]
                    art = self.art_plate(article, "K2", pt['item-K2'], pt['item-K2a'])
                    if article['platf-code'] == compte['code_client']:
                        usage = 0
                        if compte['exploitation'] == "TRUE":
                            runcae = ""
        ...
        for entree in imports.noshows.donnees:
            ...
            if id_groupe != '0':
                ...
                if self.plateforme['id_plateforme'] == article['platf-code']:
                    ...
                    util_proj = self.util_proj(entree['id_user'], compte, pt['flow-noshow'])
                    art = self.art_plate(article, code, texte, texte2)
        ...
        for entree in imports.services.donnees:
            ...
            if self.plateforme['id_plateforme'] == article['platf-code']:
                ...
                ope = [entree['id_op'], operateur['prenom'] + " " + operateur['nom'], "", entree['remarque_staff'],
                       "", "", ""]
                util_proj = self.util_proj(entree['id_user'], compte, pt['flow-srv'])
                art = self.art_plate(article, "", "", "")
                if article['platf-code'] == compte['code_client'] and compte['exploitation'] == "TRUE":
                    usage = 0
                else:
        ...
        for trf in sorted(transacts.keys()):
            for trt in sorted(transacts[trf].keys()):
                ...
                for transact in tarray:
                    ...
                    subs = self.subsides(transact, article)
    ...
    def ref_client(self, annee, mois, classe, client, compte, article):
        """
    ...
    def util_proj(self, id_user, compte, flux):
        """
    ...
    def art_plate(self, article, code_k, texte_k, texte2_k):
        """
    ...
    def subsides(self, transact, article):
        """
    ...
    def check_id_classe(dict_s, id_classe, code_client, id_machine):
        ...
        """
        if "0" in dict_s:
            if Transactions3.check_client(dict_s, "0", code_client, id_machine):
                return True
        if id_classe in dict_s:
            if Transactions3.check_client(dict_s, id_classe, code_client, id_machine):
                return True
        return False
    ...
    def check_client(dict_p, id_classe, code_client, id_machine):
        ...
        if "0" in dict_n:
            if Transactions3.check_machine(dict_n, "0", id_machine):
                return True
        if code_client in dict_n:
            if Transactions3.check_machine(dict_n, code_client, id_machine):
                return True
        return False
